...
#include "pma_hardware/interface/pma_robot.hpp"
...
std::vector<hardware_interface::CommandInterface> PmaRobotHardware::export_command_interfaces()
{
    ...
    {
        command_interfaces.emplace_back(info_.joints[i].name, hardware_interface::HW_IF_ACCELERATION, &hw_commands_accelerations_[i]);
    }
...
hardware_interface::CallbackReturn PmaRobotHardware::on_init(const hardware_interface::HardwareInfo& info)
{
    ...
    PMAI_LOG(INFO, "DOF in on_init() = %zu", dof);
    ...
    hw_commands_accelerations_.resize(dof, std::numeric_limits<double>::quiet_NaN());
    hw_states_positions_.resize(dof, std::numeric_limits<double>::quiet_NaN());
    hw_states_velocities_.resize(dof, std::numeric_limits<double>::quiet_NaN());
    hw_states_accelerations_.resize(dof, std::numeric_limits<double>::quiet_NaN());
...
hardware_interface::CallbackReturn PmaRobotHardware::on_configure(const rclcpp_lifecycle::State& /*previous_state*/)
{
    PMAI_LOG(INFO, "Entered on_configure()");

    hardware_interface::CallbackReturn rc = hardware_interface::CallbackReturn::SUCCESS;

    const size_t dof = info_.joints.size();
    PMAI_LOG(INFO, "DOF in on_configure() = %zu", dof);
...
END:
    PMAI_LOG(INFO, "on_configure() rc=%s", pma_util::callback_return_to_string(rc));
    return rc;
}
                                                       ...
                                                       const rclcpp::Duration& period)
{
    ...
    for (size_t i = 0; i < info_.joints.size(); i++)
    {
        // For a simple, simulated interface, assume commanded acceleration is
        // used, then assume the corresponding velocities and positions are met
        // to a degree
        hw_states_accelerations_[i] = hw_commands_accelerations_[i];
        hw_states_velocities_[i] += ((hw_states_accelerations_[i] * dt) / hw_slowdown_);
        hw_states_positions_[i] += ((hw_states_velocities_[i] * dt) / hw_slowdown_);
        PMAI_LOG(INFO, "Read[%zu]: dt=%.2lf, a=%.2lf, v=%.2lf, p=%.2lf", i, dt, hw_states_accelerations_[i], hw_states_velocities_[i], hw_states_positions_[i]);
    }
